...
class RagEvalPipeline:
    ...
    def relevancy_evaluator(query: str, ground_truth: str, output: dict):
        ...
        eval_source_results = [
            evaluator.evaluate(query=query, contexts=[context], response=output["response"])
            for context in output["contexts"]
        ]
        eval_source_result = [1 if result.passing else 0 for result in eval_source_results]
